const emojis = [
  '😀', '😃', '😄', '😁', '😆', '😅', '😂', '🤣', '😊', '😇',
  '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😗', '😙', '😚',
  '😋', '😛', '😝', '😜', '🤪', '🤨', '🧐', '🤓', '😎', '🤩',
  '🥳', '😏', '😒', '😞', '😔', '😟', '😕', '🙁', '☹️', '😣',
  '😖', '😫', '😩', '🥺', '😢', '😭', '😤', '😠', '😡', '🤬',
  '🤯', '😳', '🥵', '🥶', '😱', '😨', '😰', '😥', '😓', '🤗',
  '🤔', '🤭', '🤫', '🤥', '😶', '😐', '😑', '😬', '🙄', '😯',
  '😦', '😧', '😮', '😲', '🥱', '😴', '🤤', '😪', '😵', '🤐',
  '🥴', '🤢', '🤮', '🤧', '😷', '🤒', '🤕', '🤑', '🤠', '😈',
  '👿', '👹', '👺', '🤡', '💩', '👻', '💀', '☠️', '👽', '👾',
];

export const generateEmojiGrid = (level: number): string[] => {
  const gridSize = Math.min(38, 5 + Math.floor(level / 2));
  const totalEmojis = gridSize * gridSize;
  const shuffled = [...emojis].sort(() => 0.5 - Math.random());
  const selectedEmojis = shuffled.slice(0, totalEmojis - 1);
  const stillEmojiIndex = Math.floor(Math.random() * totalEmojis);
  
  return Array(totalEmojis).fill('').map((_, index) => 
    index === stillEmojiIndex ? '🎯' : selectedEmojis[index % selectedEmojis.length]
  );
};